...
from git_autosquash.git_native_complete_handler import GitNativeStrategyManager, create_git_native_handler
...
def cmd_strategy_test(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for strat in strategies_to_test:
            ...
            # Test compatibility
            compatible = GitNativeStrategyManager.validate_strategy_compatibility(git_ops, strat)
            print(f"  Compatibility: {'✓' if compatible else '✗'}")
            
            if compatible:
                # Test basic functionality
                try:
                    handler = GitNativeStrategyManager.create_handler(git_ops, strategy=strat)
                ...
                print(f"  Reason: Strategy not supported on this system")
...
def cmd_strategy_set(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if strategy == "auto":
            ...
            print("Add this to your shell profile (~/.bashrc, ~/.zshrc, etc.) to persist")
        
        # Test if the strategy is compatible
        git_ops = GitOps()
        if git_ops.is_git_repo() and strategy != "auto":
            compatible = GitNativeStrategyManager.validate_strategy_compatibility(git_ops, strategy)
            if not compatible:
...
def add_strategy_subcommands(subparsers):
    ...
    info_parser = subparsers.add_parser(
        "strategy-info",
        help="Show git-native strategy information"
    )
    ...
    test_parser = subparsers.add_parser(
        "strategy-test",
        help="Test strategy compatibility"
    )

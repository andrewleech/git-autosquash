...
class GitNativeIgnoreHandler:
    ...
    def apply_ignored_hunks(self, ignored_mappings: List[HunkTargetMapping]) -> bool:
        ...
        self.logger.info(f"Applying {len(ignored_mappings)} ignored hunks with git-native backup")
        ...
        try:
            ...
            if success:
                self.logger.info("✓ Ignored hunks successfully restored to working tree")
                return True
            else:
    ...
    def _create_comprehensive_backup(self) -> Optional[str]:
        ...
        success, stash_output = self.git_ops._run_git_command(
            "stash", "push", 
            "--include-untracked",
            "--message", "git-autosquash-comprehensive-backup"
        )
        ...
        try:
            ...
            for mapping in ignored_mappings:
                ...
                if file_path.is_absolute():
                    self.logger.error(f"Absolute file path not allowed: {mapping.hunk.file_path}")
                    return False
                ...
                try:
                    ...
                    self.logger.error(f"Path traversal detected: {mapping.hunk.file_path}")
                    return False
    ...
    def _apply_patches_with_validation(self, ignored_mappings: List[HunkTargetMapping]) -> bool:
    ...
    def _apply_patches_via_index(self, ignored_mappings: List[HunkTargetMapping]) -> bool:
        """Apply patches using git index manipulation for precise control.
            ...
            try:
                ...
                for mapping in ignored_mappings:
                    if not self._stage_hunk_to_index(mapping.hunk):
                        self.logger.error(f"Failed to stage hunk for {mapping.hunk.file_path}")
                        return False
                ...
                if success:
                    ...
                    self.logger.error(f"Git-native patch application failed: {error_msg}")
                    return False
    ...
    def _stage_hunk_to_index(self, hunk) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.warning(f"Failed to stage hunk for {hunk.file_path}: {error_msg}")
                return False
    ...
    def _generate_patch_from_index(self) -> Optional[str]:
        ...
        try:
            ...
            if success and patch_content.strip():
                self.logger.debug(f"Generated patch from index ({len(patch_content)} bytes)")
    ...
    def _force_restore_from_stash(self, stash_ref: str) -> bool:
        ...
        success, output = self.git_ops._run_git_command("checkout", stash_ref, "--", ".")

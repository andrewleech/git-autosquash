...
class HunkTargetMapping:
    ...
    def __eq__(self, other) -> bool:
        ...
        if not isinstance(other, HunkTargetMapping):
            return False
        return (
            self.hunk.file_path == other.hunk.file_path and
            self.hunk.old_start == other.hunk.old_start and
            self.hunk.old_count == other.hunk.old_count and
            self.hunk.new_start == other.hunk.new_start and
            self.hunk.new_count == other.hunk.new_count and
            self.targeting_method == other.targeting_method
        )
...
class FallbackTargetProvider:
    ...
    def get_fallback_candidates(self, file_path: str, method: TargetingMethod) -> List[str]:
        ...
        if method == TargetingMethod.FALLBACK_NEW_FILE:
            ...
            file_relevant, other_commits = self.batch_ops.get_file_relevant_commits(branch_commits, file_path)
...
class HunkTargetResolver:
    ...
    def __init__(self, git_ops: GitOps, merge_base: str):
        ...
        self.blame_engine = BlameAnalysisEngine(git_ops)
    ...
    def _resolve_single_hunk(self, hunk: DiffHunk) -> HunkTargetMapping:
        ...
        consistent_target = self.consistency_tracker.get_consistent_target(hunk.file_path)
    ...
    def _analyze_blame_consensus(self, blame_info: List[BlameInfo]) -> tuple[str, str]:
        ...
        most_frequent_commit, max_count = max(
            commit_counts.items(),
            key=lambda x: (x[1], commit_info_dict.get(x[0], type('obj', (), {'timestamp': 0})).timestamp)
        )
    ...
    def _create_fallback_mapping(
        ...
        """
        candidates = self.fallback_provider.get_fallback_candidates(hunk.file_path, method)

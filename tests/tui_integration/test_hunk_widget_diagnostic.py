...
class TestHunkWidgetDiagnostic:
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            for i, widget in enumerate(hunk_widgets[:2]):
                ...
                for rs in radio_sets:
                    components.append((f"RadioSet ({rs.id})", rs.region.height, rs.region.y))
                
                # Radio buttons
                radio_buttons = widget.query("RadioButton") 
                for rb in radio_buttons:
                    components.append((f"RadioButton ({rb.id})", rb.region.height, rb.region.y))
                
                # Action buttons container
                action_containers = widget.query(".action-buttons")
                for ac in action_containers:
                    components.append(("Action Container", ac.region.height, ac.region.y))
                ...
                print(f"Widget border/padding overhead: {widget.region.height - total_component_height}")
    ...
    @pytest.mark.asyncio
    async def test_compare_blame_vs_fallback_widgets(self, mock_commit_history_analyzer):
    ...
    @pytest.mark.asyncio  
    async def test_widget_height_before_after_changes(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            content_area_height = pilot.app.screen.query_one("#content-area").region.height
            ...
            if num_widgets > 0 and content_area_height > 0:
                ...
                widgets_that_fit = content_area_height // avg_height if avg_height > 0 else 0

...
class TestFooterButtonOverlap:
    ...
    @pytest.mark.asyncio
    async def test_buttons_visible_with_footer_different_sizes(
        ...
        for width, height in test_sizes:
            ...
            async with app.run_test(size=(width, height)) as pilot:
                ...
                print(
                    f"✓ {width}x{height}: Footer y={footer.region.y}, buttons end y={button_bottom}"
                )
    ...
    async def test_content_area_accounts_for_footer(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            print(
                f"✓ Space usage: Header={header.region.height} + Main={main_container.region.height} + Footer={footer.region.height} = {used_space}/24"
            )

...
class TestResponsiveLayout:
    ...
    @pytest.mark.asyncio
    async def test_button_positioning_small_terminal(
    ...
    @pytest.mark.asyncio
    async def test_button_positioning_large_terminal(
        self, mock_commit_history_analyzer
    ):
    ...
    @pytest.mark.asyncio
    async def test_scrolling_area_fills_available_space_small(
        self, mock_commit_history_analyzer
    ):
    ...
    @pytest.mark.asyncio
    async def test_scrolling_area_fills_available_space_large(
        self, mock_commit_history_analyzer
    ):
    ...
    async def test_horizontal_panels_split_properly(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            assert hunk_width > 10, (
                f"Hunk panel too narrow: {hunk_width} in 80px terminal"
            )
            assert diff_width > 10, (
                f"Diff panel too narrow: {diff_width} in 80px terminal"
            )
    ...
    @pytest.mark.asyncio
    async def test_very_small_terminal_graceful_handling(
        self, mock_commit_history_analyzer
    ):

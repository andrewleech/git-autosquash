...
class TextualAssertions:
    ...
    @staticmethod
    async def assert_widget_visible(pilot: Pilot, widget_id: str, timeout: float = 3.0) -> None:
    ...
    @staticmethod
    async def assert_button_at_bottom(pilot: Pilot, button_text: str, tolerance: int = 5) -> None:
    ...
    async def assert_radio_selected(pilot: Pilot, radio_label_pattern: str) -> None:
        ...
        for radio in radio_buttons:
            if re.search(radio_label_pattern, str(radio.label)):
                assert radio.value, f"Radio button matching '{radio_label_pattern}' is not selected"
                return
...
class PyteScreenAnalyzer:
    ...
    def get_color_at_position(self, row: int, col: int) -> Optional[str]:
        ...
        if (row >= len(self.screen.display) or row < 0 or 
            col >= len(self.screen.display[row]) or col < 0):
            return None
...
class MockDataGenerator:
    ...
    @staticmethod
    def create_mock_mappings(blame_count: int, fallback_count: int) -> List[Dict[str, Any]]:
        ...
        for i in range(blame_count):
            hunk = DiffHunk(
                ...
                new_start=i + 10,
                new_count=3,
                lines=[f"@@ -{i+10},3 +{i+10},3 @@", f"- old_line_{i}", f"+ new_line_{i}"],
                context_before=[],
                context_after=[]
            )
        ...
        for i in range(fallback_count):
            hunk = DiffHunk(
                ...
                new_start=i + 50,
                new_count=4,
                lines=[f"@@ -{i+50},2 +{i+50},4 @@", f"+ added_line_{i}", f"+ another_line_{i}"],
                context_before=[],
                context_after=[]
            )

...
from .helpers import PyteScreenAnalyzer, capture_textual_output
...
class TestTerminalScreenCapture:
    ...
    @pytest.mark.asyncio
    async def test_screen_capture_basic_layout(self, blame_matched_mappings, mock_commit_history_analyzer):
    ...
    @pytest.mark.asyncio
    async def test_button_positioning_at_bottom(self, mixed_mappings, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            assert button_row >= 20, f"Buttons should be near bottom, found at row {button_row}"

    @pytest.mark.asyncio
    async def test_hunk_list_diff_panel_layout(self, blame_matched_mappings, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(120, 30)) as pilot:
            ...
            assert hunk_col < sep_col < diff_col, "Panels not properly positioned around separator"

    @pytest.mark.asyncio
    async def test_color_coding_validation(self, mixed_mappings, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            assert any("Git Autosquash" in line for line in content), "Title not found in screen content"

    @pytest.mark.asyncio
    async def test_no_text_overlap_or_truncation(self, large_mappings_dataset, mock_commit_history_analyzer):
        """Test that with large datasets, text doesn't overlap or get truncated."""
        analyzer = PyteScreenAnalyzer(width=100, height=40)  # Larger screen for more data
        app = EnhancedAutoSquashApp(large_mappings_dataset[:10], mock_commit_history_analyzer)  # Subset for testing
        
        async with app.run_test(size=(100, 40)) as pilot:
            # Simulate output with many hunks
            test_lines = []
            test_lines.append("Git Autosquash")
            test_lines.append("Progress Summary: 10 hunks - 7 automatic targets, 3 manual selection")
            test_lines.append("─" * 98)
            test_lines.append("Hunks" + " " * 35 + "│" + " " * 10 + "Diff Preview")
            
            # Add multiple hunk entries
            for i in range(10):
                hunk_line = f"test_file_{i % 3}.py @@ -{i+1},3 +{i+1},3" + " " * 10 + f"│ @@ -{i+1},3 +{i+1},3 @@"
                test_lines.append(hunk_line)
                commit_line = f" ● commit_{i:08d}: Fix issue in component" + " " * 5 + "│   - old_code_{i}"
            ...
            for i, line in enumerate(content):
                assert len(line) <= 100, f"Line {i} exceeds screen width: {len(line)} chars"

    @pytest.mark.asyncio
    async def test_scrolling_behavior_with_many_hunks(self, large_mappings_dataset, mock_commit_history_analyzer):
        """Test scrolling behavior when there are more hunks than fit on screen."""
        analyzer = PyteScreenAnalyzer(width=80, height=24)
        app = EnhancedAutoSquashApp(large_mappings_dataset[:20], mock_commit_history_analyzer)  
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            content = analyzer.get_screen_content()
            hunk_lines = [line for line in content if "Hunk" in line and "test_file_" in line]
            
            # Should show subset of hunks, not all 20
            assert len(hunk_lines) < 20, "All hunks shown at once - scrolling may not be working"
...
class TestLayoutValidation:
    ...
    @pytest.mark.asyncio
    async def test_exact_widget_positioning(self, blame_matched_mappings, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            lines[4] = "Progress Summary: 2 hunks - 2 automatic targets, 0 manual selection"
            lines[6] = "─" * 78  # Separator
            lines[8] = "Hunks" + " " * 30 + "│" + " " * 10 + "Diff Preview"
            lines[10] = "shared/runtime/pyexec.c @@ -87,7" + " " * 8 + "│" + " " * 5 + "@@ -87,7 +87,7 @@"
    ...
    @pytest.mark.asyncio
    async def test_widget_bounds_no_overlap(self, mixed_mappings, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(100, 30)) as pilot:
            ...
            test_output = (
                ...
                "shared/runtime/pyexec.c @@ -87,7" + " " * 15 + "│" + " " * 10 + "@@ -87,7 +87,7 @@\n"
                "Target commit (auto-detected):" + " " * 14 + "│\n"
                " ● d59d2691: Fix context handling" + " " * 12 + "│" + " " * 10 + "-  #if MICROPY_PY___FILE__\n"
                "⚠ Manual Selection Required" + " " * 18 + "│" + " " * 10 + "+  #if MICROPY_MODULE___FILE__\n"
            ...
            # Verify no overlaps
            assert analyzer.verify_no_overlap(widgets), "Widgets are overlapping on screen"

    @pytest.mark.asyncio
    async def test_box_drawing_characters_alignment(self, blame_matched_mappings, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            for row_idx, line in enumerate(content):
                if "│" in line:
                    ...
                    assert len(bar_positions) > 0, f"No vertical bars found in line {row_idx}"
...
class TestPerformanceValidation:
    ...
    @pytest.mark.asyncio
    async def test_large_dataset_rendering_performance(self, large_mappings_dataset, mock_commit_history_analyzer):
        """Test that large datasets render without performance issues."""
        analyzer = PyteScreenAnalyzer(width=120, height=50)
        app = EnhancedAutoSquashApp(large_mappings_dataset, mock_commit_history_analyzer)
    ...
    @pytest.mark.asyncio 
    async def test_responsive_layout_terminal_resize(self, mixed_mappings, mock_commit_history_analyzer):
        ...
        large_output = (
            ...
            "shared/runtime/pyexec.c @@ -87,7 +87,7" + " " * 20 + "│" + " " * 10 + "@@ -87,7 +87,7 @@\n"
            "● d59d2691: Fix context handling in pyexec module" + " " * 5 + "│" + " " * 10 + "- #if MICROPY_PY___FILE__\n"
            "Approve All & Continue │ Continue with Selected │ Cancel\n"
        )
        ...
        assert any("Git Autosquash" in line for line in small_content), "Small layout broken"
        assert any("Git Autosquash" in line for line in large_content), "Large layout broken"
        ...
        assert large_text_len > small_text_len, "Large layout should have more content than small"
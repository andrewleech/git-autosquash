    ...
    async def test_all_improvements_summary(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            for i, widget in enumerate(hunk_widgets[:3]):
                ...
                if action_radiosets:
                    ...
                    assert len(selected) <= 1, (
                        f"Widget {i + 1}: Multiple action buttons selected"
                    )
                    mutually_exclusive_widgets += 1
                    print(
                        f"   Widget {i + 1}: Action selector with {len(radio_buttons)} options (mutually exclusive)"
                    )
                ...
                if target_radiosets:
                    ...
                    selected = [r for r in radio_buttons if r.value]
                    assert len(selected) <= 1, (
                        f"Widget {i + 1}: Multiple target buttons selected"
                    )
                    mutually_exclusive_widgets += 1
                    print(
                        f"   Widget {i + 1}: Target selector with {len(radio_buttons)} options (mutually exclusive)"
                    )

            print(
                f"   ✓ {mutually_exclusive_widgets} widgets have mutually exclusive selection"
            )
            ...
            if gaps:
                ...
                normal_gaps = [
                    g for g in gaps if g <= 5
                ]  # Filter out section separators
                if normal_gaps:
                    max_normal_gap = max(normal_gaps)
                    assert max_normal_gap <= 2, (
                        f"Normal gap too large: {max_normal_gap}"
                    )
            ...
            if content_height > 0:
                ...
                assert widgets_that_fit >= 1, (
                    f"Widgets too large: only {widgets_that_fit} fit"
                )
            ...
            assert checkbox_count == 0, (
                f"Found {checkbox_count} non-exclusive checkboxes"
            )

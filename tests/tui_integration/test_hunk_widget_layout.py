...
class TestHunkWidgetLayout:
    ...
    async def test_hunk_widget_compact_layout(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            assert len(hunk_widgets) >= 3, f"Expected at least 3 hunk widgets, found {len(hunk_widgets)}"
            
            print(f"\\n=== Hunk Widget Layout Analysis ===")
            
            for i, widget in enumerate(hunk_widgets[:3]):  # Check first 3
                widget_height = widget.region.height
                print(f"Hunk {i+1}: height={widget_height}, y={widget.region.y}-{widget.region.bottom-1}")
                
                # Verify reasonable height (improved from previous version)
                assert widget_height <= 10, (
    ...
    @pytest.mark.asyncio
    async def test_action_buttons_are_mutually_exclusive(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            for hunk_widget in hunk_widgets:
                ...
                if action_radiosets:
                    ...
                    assert len(radio_buttons) == 2, f"Expected 2 action radio buttons, found {len(radio_buttons)}"
                ...
                if target_radiosets:
                    ...
                    found_mutual_exclusivity = True
                    print(f"✓ Found mutually exclusive target selection ({len(radio_buttons)} options)")
    ...
    async def test_hunk_widget_spacing_consistency(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 24)) as pilot:
            ...
            if gaps:
                ...
                assert 0 <= avg_gap <= 2, (
                    f"Gaps too large: average {avg_gap:.1f} rows"
                )
    ...
    async def test_hunk_widget_no_wasted_space(self, mock_commit_history_analyzer):
        ...
        async with app.run_test(size=(80, 20)) as pilot:  # Smaller terminal to test efficiency
            await pilot.pause(0.1)
            ...
            for i, widget in enumerate(hunk_widgets[:2]):
                ...
                content_elements = len(header) + len(commit_info) + len(action_buttons) + len(radio_buttons)
                ...
                print(f"Hunk {i+1}: {widget_height} rows, {content_elements} elements, ratio: {efficiency_ratio:.1f}")

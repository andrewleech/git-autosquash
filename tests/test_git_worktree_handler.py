...
class TestGitWorktreeIgnoreHandler:
    ...
    def test_successful_worktree_creation(self, mock_mkdtemp):
        ...
        mock_mkdtemp.assert_called_once_with(
            prefix="git-autosquash-", suffix="-worktree"
        )
    ...
    def test_worktree_creation_failure_cleanup(self, mock_rmtree, mock_mkdtemp):
        ...
        mock_rmtree.assert_called_once_with(
            "/tmp/test-worktree-failed", ignore_errors=True
        )
    ...
    def test_successful_apply_with_worktree(self):
        ...
        with (
            ...
            mock_worktree_git_ops._run_git_command_with_input.return_value = (
                True,
                "",
            )  # apply hunk
            ...
            assert (
                self.git_ops._run_git_command_with_input.call_count == 2
            )  # check + apply
    ...
    def test_path_validation_security(self):
        ...
        self.git_ops._run_git_command.return_value = (
            True,
            "worktree help with add command",
        )
    ...
    def test_worktree_cleanup_with_fallback(self):
        ...
        with patch.object(
            self.handler, "_manual_worktree_cleanup"
        ) as mock_manual_cleanup:
    ...
    def test_manual_worktree_cleanup_graceful_handling(self):
        ...
        try:
            ...
            pytest.fail(
                f"Manual cleanup should handle exceptions gracefully, but raised: {e}"
            )

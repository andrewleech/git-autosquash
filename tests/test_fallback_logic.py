        ...
        with patch.object(self.analyzer, "_get_branch_commits") as mock_commits:
            ...
            def mock_get_info(commit_hash):
                if commit_hash == "recent":
                    return CommitInfo(
                        commit_hash, "rec123", "Recent", "Author", 1640995300, False
                    )
                elif commit_hash == "old":
                    return CommitInfo(
                        commit_hash, "old123", "Old", "Author", 1640995100, False
                    )
                else:  # merge
                    return CommitInfo(
                        commit_hash, "mer123", "Merge", "Author", 1640995200, True
                    )

            with patch.object(
                self.analyzer, "get_commit_info", side_effect=mock_get_info
            ):
...
class TestFallbackIntegration:
    ...
    def test_mixed_blame_and_fallback_scenario(self):
        ...
        self.git_ops._run_git_command.side_effect = [
            ...
            (
                True,
                "target123 (author 2023-01-01 10:00:00 +0000 10) old line",
            ),  # Blame for first file
    ...
    def test_file_consistency_across_multiple_hunks(self):
        ...
        self.git_ops._run_git_command.side_effect = [
            ...
            (
                True,
                "target456 (author 2023-01-01 10:00:00 +0000 5) line1",
            ),  # Blame first hunk

...
from git_autosquash.blame_analyzer import BlameAnalyzer, HunkTargetMapping
...
class TestPerformanceBenchmarks:
    ...
    def test_patch_creation_performance(self, num_hunks: int) -> None:
        ...
        assert execution_time < 1.0, f"Patch creation too slow for {num_hunks} hunks: {execution_time:.3f}s"
        
        # Verify patch contains expected content
        assert len(patch_content) > 0
        assert f"file_0.py" in patch_content
    ...
    def test_ui_state_controller_performance(self, num_mappings: int) -> None:
        ...
        assert approve_time < 0.1, f"Approve all too slow for {num_mappings} mappings: {approve_time:.3f}s"
        assert lookup_time < 0.01, f"Individual lookups too slow: {lookup_time:.3f}s for 100 lookups"
        assert toggle_time < 0.1, f"Ignore toggle too slow for {num_mappings} mappings: {toggle_time:.3f}s"
        ...
        print(f"✓ UI operations for {num_mappings} mappings: approve={approve_time:.3f}s, lookup={lookup_time:.3f}s, toggle={toggle_time:.3f}s")
    ...
    def test_mapping_processing_performance(self, num_mappings: int) -> None:
        ...
        assert execution_time < 0.5, f"Mapping processing too slow for {num_mappings} mappings: {execution_time:.3f}s"
    ...
    def test_large_repository_end_to_end_simulation(self) -> None:
        ...
        git_ops._run_git_command_with_input.return_value = (True, "Applied successfully")
        ...
        assert execution_time < 1.0, f"Large repository simulation too slow: {execution_time:.3f}s"
        
        # Verify git-native approach: n hunks staged + validation + final apply
        expected_calls = num_hunks + 2  # stage each hunk individually + validate + apply
        ...
        print(f"✓ Large repository simulation ({num_hunks} hunks) completed in {execution_time:.3f}s")
    ...
    def test_memory_usage_with_large_datasets(self) -> None:
        ...
        assert objects_created < max_expected_objects, f"Memory usage too high: {objects_created} objects for {num_hunks} hunks"
    ...
    def test_concurrent_access_performance(self) -> None:
        ...
        assert execution_time < 0.1, f"Concurrent access simulation too slow: {execution_time:.3f}s"
